alter table public.business_map_nodes
  add column if not exists business_node boolean not null default false;

alter table public.business_map_nodes
  add column if not exists decision_node boolean not null default false;

update public.business_map_nodes node
set business_node = true
where node.parent_id is null
  and not node.standalone_node
  and not exists (
    select 1
    from public.business_map_nodes sibling
    where sibling.map_id = node.map_id
      and sibling.parent_id is null
      and not sibling.standalone_node
...
begin
  ...
  insert into public.business_map_nodes (
    ...
    automated_work, department_node, business_node, decision_node, tool_node, standalone_node, node_shape, node_color, placement,
  ...
  select
    ...
    node.repeated_work, node.human_branch, node.human_ai_mix, node.automated_work, node.department_node, node.business_node, node.decision_node,
  ...
  from jsonb_to_recordset(p_nodes) as node(
    ...
    automated_work boolean, department_node boolean, business_node boolean, decision_node boolean, tool_node boolean, standalone_node boolean,
...
begin
  ...
  select
    ...
    source.human_ai_mix, source.automated_work, source.department_node, source.business_node, source.decision_node, source.tool_node,
