alter table public.business_map_nodes
  add column if not exists human_ai_mix boolean not null default false;
...
begin
  ...
  insert into public.business_map_nodes (
    ...
    tool_node, standalone_node, node_shape, node_color, placement, position_locked, updated_at
  )
  select
    ...
    node.repeated_work, node.human_branch, node.human_ai_mix, node.tool_node,
    node.standalone_node, node.node_shape, node.node_color, node.placement,
  ...
  from jsonb_to_recordset(p_nodes) as node(
    ...
    tool_node boolean, standalone_node boolean, node_shape text, node_color text,
    placement text, position_locked boolean
  )
...
begin
  ...
  select
    ...
    source.human_ai_mix, source.tool_node, source.standalone_node, source.node_shape,
    source.node_color, source.placement, true, now()
  from public.business_map_nodes source
  where source.map_id = p_source_map_id;
