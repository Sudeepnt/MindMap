alter table public.business_map_nodes
  add column if not exists agency_node boolean not null default false,
  add column if not exists client_node boolean not null default false;
...
begin
  ...
  insert into public.business_map_nodes (
    ...
    automated_work, department_node, business_node, decision_node, handoff_node, database_node, agency_node, client_node, tool_node, standalone_node, node_shape, node_color, placement,
  ...
  select
    ...
    node.repeated_work, node.human_branch, node.human_ai_mix, node.automated_work, node.department_node, node.business_node, node.decision_node, node.handoff_node,
    node.database_node, node.agency_node, node.client_node, node.tool_node, node.standalone_node, node.node_shape, node.node_color,
  ...
  from jsonb_to_recordset(p_nodes) as node(
    ...
    automated_work boolean, department_node boolean, business_node boolean, decision_node boolean, handoff_node boolean, database_node boolean, agency_node boolean, client_node boolean, tool_node boolean, standalone_node boolean,
...
begin
  ...
  select
    ...
    source.human_ai_mix, source.automated_work, source.department_node, source.business_node, source.decision_node, source.handoff_node, source.database_node, source.agency_node, source.client_node, source.tool_node,
