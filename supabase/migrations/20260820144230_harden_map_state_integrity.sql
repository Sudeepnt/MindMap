...
begin
  ...
  if exists (
    ...
    where child.parent_id is not null
      and not exists (
        select 1
        from jsonb_to_recordset(p_nodes) as parent(id text)
  ...
  select
    ...
    node.repeated_work, node.human_branch, node.tool_node, node.standalone_node,
    node.node_shape, node.node_color, node.placement, node.position_locked, now()
  from jsonb_to_recordset(p_nodes) as node(
    ...
    repeated_work boolean, human_branch boolean, tool_node boolean,
    standalone_node boolean, node_shape text, node_color text, placement text,
