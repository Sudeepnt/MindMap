alter table public.business_map_nodes
  add column if not exists department_node boolean not null default false;
...
begin
  ...
  insert into public.business_map_nodes (
    ...
    automated_work, department_node, tool_node, standalone_node, node_shape, node_color, placement,
  ...
  select
    ...
    node.repeated_work, node.human_branch, node.human_ai_mix, node.automated_work, node.department_node,
  ...
  from jsonb_to_recordset(p_nodes) as node(
    ...
    automated_work boolean, department_node boolean, tool_node boolean, standalone_node boolean,
...
begin
  ...
  select
    ...
    source.human_ai_mix, source.automated_work, source.department_node, source.tool_node,
